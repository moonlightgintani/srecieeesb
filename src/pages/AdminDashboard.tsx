                          ...
                          return monthlyEventData.map((item, idx) => (
                            <div key={idx} className="flex-1 flex flex-col items-center gap-2 group">
                              <div className="w-full bg-slate-50 rounded-t-md relative h-48 flex items-end overflow-hidden">
                                ...
                                >
                                  <span className="absolute -top-7 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] font-bold rounded px-1.5 py-0.5 opacity-0 group-hover:opacity-100 transition pointer-events-none whitespace-nowrap">
                                    {item.val} Events
                                  </span>
                                </div>
                              </div>
                    ...
                    <div>
                      ...
                      <div className="flex justify-center items-center py-6 relative">
                        ...
                        <svg className="w-40 h-40 transform -rotate-90">
                          ...
                          <circle cx="80" cy="80" r="65" stroke="#2563eb" strokeWidth="14" fill="transparent"
                            strokeDasharray="408" strokeDashoffset={Math.round(408 - (408 * memberDistribution.studentPercent) / 100)} strokeLinecap="round" className="transition-all duration-500" />
                        </svg>
                        <div className="absolute flex flex-col items-center">
                          <span className="text-3xl font-black text-slate-800">{memberDistribution.studentPercent}%</span>
                      ...
                      <div className="space-y-3 pt-6 border-t border-slate-100">
                        <div className="flex items-center justify-between text-xs font-semibold">
                          ...
                          <span className="text-slate-800">{memberDistribution.studentPercent}%</span>
                        </div>
                        <div className="flex items-center justify-between text-xs font-semibold">
                          <span className="flex items-center gap-2 text-slate-500">
                            <span className="w-3 h-3 rounded-full bg-slate-200"></span> Professional ({memberDistribution.profCount})
                          </span>
                          <span className="text-slate-800">{memberDistribution.profPercent}%</span>
                        </div>
                      </div>
                    </div>
                  </div>
